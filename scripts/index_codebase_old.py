...
# The Answer: Not exactly "trained" in the machine learning sense, but "programmed." An AST parser uses a Formal Grammar (usually via a library like tree-sitter). It’s a deterministic engine that knows the rules of Java syntax. It doesn't "guess" where a function starts; it builds a literal tree of the code.
# Interview Tip: When they ask about your indexing, say: "I used a deterministic AST parser to ensure syntactic units like classes and methods remained atomic, rather than using a naive character-based splitter which would likely slice a function in half."
...
# 1. Load Python Files
loader = GenericLoader.from_filesystem(
    "./my-python-app", # Your new python repo path
    glob="**/*.py",
    suffixes=[".py"],
...
# 2. Split with Python Awareness
python_splitter = RecursiveCharacterTextSplitter.from_language(
    language=Language.PYTHON, 
    chunk_size=1500, # Python is often less verbose than Java, smaller chunks work well
    chunk_overlap=150
)
